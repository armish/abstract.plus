#!/usr/bin/env python3
"""
AACR 2026 Abstract Annotator - Conference Web Application
...
<head>
    ...
    <title>AACR 2026 Abstract Annotator</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <header>
        <div class="container">
            <h1>AACR 2026 Abstract Annotator</h1>
            <div style="text-align: center; margin-top: 10px;">
                <a href="https://www.abstractsonline.com/pp8/#!/21436/" target="_blank" style="color: #ecf0f1; text-decoration: none; font-size: 0.9rem;">
                    View AACR 2026 Program →
                </a>
            </div>
        </div>
    </header>
...
def load_data():
    ...
    try:
        # Look for the AACR Excel file in the same directory as this script
        script_dir = os.path.dirname(os.path.abspath(__file__))
        filename = 'aacr2026_abstracts.xlsx'
        ...
        # Treat "no abstract" placeholder as empty
        no_abstract_msg = 'There is no abstract associated with this presentation.'
        df.loc[df['Abstract'] == no_abstract_msg, 'Abstract'] = ''
        ...
        # Sort by Abstract ID
        df = df.sort_values('Abstract ID', ascending=True)
...
def download_current_view():
    ...
    # Reorder columns to match display - AACR columns
    cols = ['Abstract ID', 'Session ID', 'Session Title', 'Start', 'End', 'Title', 'Presenter', 'Authors', 'Abstract', 'Abstract link']
    ...
    filename = f"aacr_abstracts_filtered_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
def download_results(task_id):
    ...
    # Reorder columns to match display - AACR columns
    base_cols = ['Abstract ID', 'Session ID', 'Session Title', 'Start', 'End', 'Title', 'Presenter', 'Authors', 'Abstract', 'Abstract link']
...
if __name__ == '__main__':
    ...
    print("\n" + "="*50)
    print("AACR 2026 Abstract Annotator")
    print("="*50)
    print("\nStarting web server...")
