...
<head>
    ...
    <title>ACR 2025 Abstract Annotator</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <header>
        <div class="container">
            <h1>ACR 2025 Abstract Annotator</h1>
            <div style="text-align: center; margin-top: 10px;">
                <a href="https://acrabstracts.org/meetings/acr-convergence-2025/" target="_blank" style="color: #ecf0f1; text-decoration: none; font-size: 0.9rem;">
                    View Original ACR Convergence 2025 →
                </a>
            </div>
        </div>
    </header>
    ...
    <script>
        ...
        function loadAbstracts(page = 1) {
            ...
            fetch('/api/abstracts?page=' + page + '&per_page=' + perPage + '&search=' + encodeURIComponent(search) + '&show_empty=' + showEmpty)
                ...
                .then(data => {
                    ...
                    thead.innerHTML = '<th>Abstract #</th><th>Keywords</th><th>Author</th><th>Abstract Title</th><th>Abstract</th><th>Link</th>' +
        ...
        function formatAbstractText(text) {
            ...
            const headers = ['Background/Purpose', 'Methods', 'Results', 'Conclusion', 'Disclosures'];
...
def load_data():
    ...
    try:
        # Look for the ACR Excel file in the same directory as this script
        script_dir = os.path.dirname(os.path.abspath(__file__))
        filename = '2025 ACR - Abstracts.xlsx'
        ...
        # Map ACR columns to standardized column names
        column_mapping = {
        ...
        def extract_first_author(authors_text):
            if not authors_text or pd.isna(authors_text):
                return ''
            # First author is typically before the first comma or "and"
            # Format: "FirstName LastName, ..." or "FirstName LastName 1, ..."
            parts = str(authors_text).split(',')
            if parts:
                first_part = parts[0].strip()
                # Remove any trailing numbers (superscripts for affiliations)
                import re
                first_part = re.sub(r'\s+\d+\s*$', '', first_part)
                return first_part
            return ''

        df['First Author'] = df['Authors'].apply(extract_first_author)
...
def download_current_view():
    ...
    cols = ['Abstract #', 'Keywords', 'First Author', 'Abstract title', 'Abstract', 'Link']
    ...
    filename = f"acr_abstracts_filtered_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
def download_results(task_id):
    ...
    # Reorder columns to match display
    base_cols = ['Abstract #', 'Keywords', 'First Author', 'Abstract title', 'Abstract', 'Link']
...
if __name__ == '__main__':
    ...
    print("\n" + "="*50)
    print("ACR 2025 Abstract Annotator")
    print("="*50)
    print("\nStarting web server...")
