...
<head>
    ...
    <title>SITC 2025 Abstract Annotator</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <header>
        <div class="container">
            <h1>SITC 2025 Abstract Annotator</h1>
            <div style="text-align: center; margin-top: 10px;">
                <a href="https://www.sitcancer.org/2025/abstracts/titles-and-publications" target="_blank" style="color: #ecf0f1; text-decoration: none; font-size: 0.9rem;">
                    View Original SITC 2025 Abstracts →
                </a>
            </div>
        </div>
    </header>
    ...
    <script>
        ...
        function loadAbstracts(page = 1) {
            ...
            fetch('/api/abstracts?page=' + page + '&per_page=' + perPage + '&search=' + encodeURIComponent(search) + '&show_empty=' + showEmpty)
                ...
                .then(data => {
                    ...
                    thead.innerHTML = '<th>Abstract #</th><th>Keywords</th><th>Primary Category</th><th>Presentation Type</th><th>Author</th><th>Abstract Title</th><th>Abstract</th><th>Link</th>' +
                    ...
                    data.data.forEach(row => {
                        ...
                        const linkHtml = row['Link'] ? '<a href="' + escapeHtml(row['Link']) + '" target="_blank" style="color: #3498db; text-decoration: none;">View</a>' : '-';

                        tr.innerHTML = '<td>' + (row['Abstract #'] || '-') + '</td>' +
                            '<td>' + (row['Keywords'] || '-') + '</td>' +
                            '<td>' + (row['Primary Category'] || '-') + '</td>' +
                            '<td>' + (row['Presentation Type'] || '-') + '</td>' +
        ...
        function showAbstractModal(row) {
            ...
            if (row['Link']) {
                infoHTML += `<div class="modal-info-item"><span class="modal-info-label">Link:</span> <a href="${row['Link']}" target="_blank" style="color: #3498db;">${row['Link']}</a></div>`;
            }
...
def load_data():
    ...
    try:
        # Look for the SITC Excel file in the same directory as this script
        script_dir = os.path.dirname(os.path.abspath(__file__))
        filename = '2025 SITC - Abstracts.xlsx'
        ...
        # Map SITC columns to standardized column names
        column_mapping = {
        ...
        # Keep SITC-specific columns as they are
        # Primary Category, Presentation Type, Abstract Type, Keywords, Authors
        # These are already properly named in the SITC spreadsheet
        ...
        # Sort by Abstract # (convert to numeric for proper sorting)
        df['Abstract #'] = pd.to_numeric(df['Abstract #'], errors='coerce')
        df = df.sort_values('Abstract #', ascending=True)
        df['Abstract #'] = df['Abstract #'].astype(str).str.replace('.0', '', regex=False)
...
def download_current_view():
    ...
    cols = ['Abstract #', 'Keywords', 'Primary Category', 'Presentation Type', 'First Author', 'Abstract title', 'Abstract', 'Link']
    ...
    filename = f"sitc_abstracts_filtered_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
if __name__ == '__main__':
    ...
    print("\n" + "="*50)
    print("SITC 2025 Abstract Annotator")
    print("="*50)
    print("\nStarting web server...")
