#!/usr/bin/env python3
"""
ASCO 2025 Abstract Annotator - All-in-One Bundle
...
<head>
    ...
    <title>ASCO 2025 Abstract Annotator</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <header>
        <div class="container">
            <h1>ASCO 2025 Abstract Annotator</h1>
        </div>
    </header>
                ...
                <div class="settings-content" id="annotationAdvancedSettings">
                    <div class="control-group">
                        ...
                        <select id="modelSelect">
                            ...
                            <option value="o1-mini">o1-mini - Faster, more affordable reasoning</option>
                            <option value="gpt-4-1106-preview">GPT-4.1 mini - Balanced intelligence</option>
                            <option value="gpt-4-0125-preview" selected>GPT-4.1 nano - Fastest, most cost-effective</option>
                            <option value="o3-mini">o3-mini - Small model alternative to o3</option>
                        </select>
                    </div>
        ...
        function loadAbstracts(page = 1) {
            ...
            fetch('/api/abstracts?page=' + page + '&per_page=' + perPage + '&search=' + encodeURIComponent(search) + '&show_empty=' + showEmpty)
                ...
                .then(data => {
                    ...
                    thead.innerHTML = '<th>Abstract #</th><th>Track</th><th>Author</th><th>Abstract Title</th><th>Abstract</th><th>Link</th>' +
                    ...
                    data.data.forEach(row => {
                        ...
                        tr.innerHTML = '<td>' + (row['Abstract #'] || '-') + '</td>' +
                            ...
                            '<td class="abstract-cell">' + 
                                (hasAbstract ? 
                                    '<div class="abstract-preview">' + escapeHtml(abstractText) + '</div>' +
                                    '<button class="expand-button" onclick="toggleAbstract(this)">+</button>' : 
                                    '-') + 
                            '</td>' +
                            '<td>' + (row['Link'] ? '<a href="' + row['Link'] + '" target="_blank">View</a>' : '-') + '</td>' +
    ...
    try:
        ...
        if df is None:
            ...
            if xlsx_files:
                ...
                print(f"Please place your ASCO abstracts Excel file in: {script_dir}")
                return pd.DataFrame()
...
def download_current_view():
    ...
    cols = ['Abstract #', 'Track', 'First Author', 'Abstract title', 'Abstract', 'Link']
...
def download_results(task_id):
    ...
    # Reorder columns to match display
    base_cols = ['Abstract #', 'Track', 'First Author', 'Abstract title', 'Abstract', 'Link']
...
def get_models():
    ...
    return jsonify({
        'models': [
            ...
            {'id': 'o1-mini', 'name': 'o1-mini - Faster, more affordable reasoning'},
            {'id': 'gpt-4-1106-preview', 'name': 'GPT-4.1 mini - Balanced intelligence'},
            {'id': 'gpt-4-0125-preview', 'name': 'GPT-4.1 nano - Fastest, most cost-effective'},
            {'id': 'o3-mini', 'name': 'o3-mini - Small model alternative to o3'}
        ]
    })
...
if __name__ == '__main__':
    print("\n" + "="*50)
    print("ASCO 2025 Abstract Annotator")
    print("="*50)
    print("\nStarting web server...")
