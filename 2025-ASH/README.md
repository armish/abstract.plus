# SITC 2025 Abstract Annotator

A web-based tool for searching, viewing, and annotating SITC 2025 conference abstracts using OpenAI's language models. This tool provides an intuitive interface to browse, search, and analyze abstracts from the Society for Immunotherapy of Cancer (SITC) 2025 meeting.

## Features

- **Search & Filter**: Search abstracts by keywords (semicolon-separated) with matching keyword tracking
- **Abstract Viewing**:
  - Inline expansion with formatted section headers (Background, Methods, Results, Conclusions, Ethics Approval, Trial Registration, References, Disclosures)
  - Full-screen modal view with keyword highlighting
  - Direct links to original abstracts on sitcancer.org
...
1. **Prepare your data file**: This application is configured to use `2025 SITC - Abstracts.xlsx` which should be placed in the same directory as `conference-webapp.py`.

2. **Excel file format**: The application expects these columns:
   - `Abstract Number` - Abstract ID/Number
   - `Title` - Abstract title
   - `Presenting Author` - First author information
   - `Authors` - Full author list
   - `Keywords` - Research keywords/topics
   - `Primary Category` - Main conference track/category
   - `Presentation Type` - Poster, Oral, etc.
   - `Abstract Type` - Regular, Late-breaking, etc.
   - `abstract text` - Full abstract text
   - `link` - URL to the original abstract on sitcancer.org

   The application automatically maps these to standardized column names and displays:
   - `Abstract #`
   - `Keywords`
   - `Primary Category`
   - `Presentation Type`
   - `Author` (First Author extracted automatically)
   - `Abstract title`
   - `Abstract` (with formatted section headers)
   - `Link`
...
4. Search works across all fields: Abstract #, Title, Authors, Keywords, Primary Category, Presentation Type, and Abstract text
...
- **Modal View**: Click the purple `⊡` button to open the abstract in a full-screen modal with:
  - Formatted section headers (Background, Methods, Results, Conclusions, Ethics Approval, Trial Registration, References, Disclosures)
  - Keyword highlighting (if searched)
  - Abstract metadata (ID, Author, Keywords, Primary Category)
  - Direct link to original abstract on sitcancer.org
- **Original Abstract**: Click the "View" link in the Link column to open the original abstract on sitcancer.org in a new tab
...
- Files are named with timestamps for easy organization (e.g., `sitc_abstracts_filtered_20251104_153000.csv`)
...
This application is specifically configured for SITC 2025 abstracts:
- **Data file**: `2025 SITC - Abstracts.xlsx`
- **Total abstracts**: 1,286 (1,236 with abstract text)
- **Conference**: Society for Immunotherapy of Cancer (SITC) 2025
- **Original source**: https://www.sitcancer.org/2025/abstracts/titles-and-publications
...
- Check that the file name matches `2025 SITC - Abstracts.xlsx`
...
- **Column Display**: Abstract #, Keywords, Primary Category, Presentation Type, Author, Abstract title, Abstract, Link
...
Built for SITC 2025 abstract analysis. Adapted from the ACR 2025 and ESMO 2025 Abstract Annotator framework.
