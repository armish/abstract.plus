#!/usr/bin/env python3
"""
ASH 2025 Abstract Annotator - Conference Web Application
...
<head>
    ...
    <title>ASH 2025 Abstract Annotator</title>
    <style>
        * {
            margin: 0;
            padding: 0;
        ...
        /* Column widths for SITC table */
        th:nth-child(1), td:nth-child(1) { /* Abstract # */
            width: 80px;
            min-width: 80px;
        }

        th:nth-child(2), td:nth-child(2) { /* Keywords */
            width: 150px;
            min-width: 120px;
        }

        th:nth-child(3), td:nth-child(3) { /* Primary Category */
            width: 150px;
            min-width: 120px;
        }

        th:nth-child(4), td:nth-child(4) { /* Presentation Type */
            width: 120px;
            min-width: 100px;
        }

        th:nth-child(5), td:nth-child(5) { /* Author */
            width: 120px;
            min-width: 100px;
        }

        th:nth-child(6), td:nth-child(6) { /* Abstract title */
            width: 200px;
            min-width: 150px;
        }

        th:nth-child(7), td:nth-child(7) { /* Abstract */
            width: 300px;
            min-width: 250px;
        }

        th:nth-child(8), td:nth-child(8) { /* Link */
            width: 80px;
            min-width: 60px;
            text-align: center;
        }

        th:nth-child(9), td:nth-child(9) { /* Matched Keywords */
            width: 120px;
            min-width: 100px;
...
<body>
    <header>
        <div class="container">
            <h1>ASH 2025 Abstract Annotator</h1>
            <div style="text-align: center; margin-top: 10px;">
                <a href="https://submit.hematology.org/program" target="_blank" style="color: #ecf0f1; text-decoration: none; font-size: 0.9rem;">
                    View ASH 2025 Program →
                </a>
            </div>
        </div>
    </header>
    ...
    <script>
        ...
        function loadAbstracts(page = 1) {
            ...
            fetch('/api/abstracts?page=' + page + '&per_page=' + perPage + '&search=' + encodeURIComponent(search) + '&show_empty=' + showEmpty)
                ...
                .then(data => {
                    ...
                    thead.innerHTML = '<th>Abstract #</th><th>Title</th><th>Category</th><th>First Author</th><th>Keywords</th><th>Abstract</th>' +
                    ...
                    data.data.forEach(row => {
                        ...
                        // Combine Category and Subcategory with line break
                        const categoryCell = (row['Category'] || '') + (row['Subcategory'] ? '<br>' + escapeHtml(row['Subcategory']) : '');
        ...
        function loadAnnotatedResults(page = 1) {
            ...
            fetch('/api/annotated/' + currentTaskId + '?page=' + page + '&per_page=' + perPage)
                ...
                .then(data => {
                    ...
                    // Add standard columns for ASH
                    const standardColumns = [
                        {name: 'Abstract #', field: 'Abstract #'},
                        {name: 'Title', field: 'Abstract title'},
                        {name: 'Category', field: 'Category'},
                        {name: 'First Author', field: 'First Author'},
        ...
        function showAbstractModal(row) {
            ...
            // Set info - show ALL columns from the table
            let infoHTML = '';
            if (row['Abstract #']) {
                infoHTML += `<div class="modal-info-item"><span class="modal-info-label">Abstract #:</span> ${row['Abstract #']}</div>`;
            }
            if (row['Order']) {
                infoHTML += `<div class="modal-info-item"><span class="modal-info-label">Order:</span> ${row['Order']}</div>`;
            }
            if (row['Category']) {
                infoHTML += `<div class="modal-info-item"><span class="modal-info-label">Category:</span> ${row['Category']}</div>`;
            }
            if (row['Subcategory']) {
                infoHTML += `<div class="modal-info-item"><span class="modal-info-label">Subcategory:</span> ${row['Subcategory']}</div>`;
            }
            if (row['Authors']) {
                infoHTML += `<div class="modal-info-item"><span class="modal-info-label">Authors:</span> ${row['Authors']}</div>`;
            }
            if (row['First Author']) {
                infoHTML += `<div class="modal-info-item"><span class="modal-info-label">First Author:</span> ${row['First Author']}</div>`;
            }
            if (row['Keywords']) {
                infoHTML += `<div class="modal-info-item"><span class="modal-info-label">Keywords:</span> ${row['Keywords']}</div>`;
            }
            if (row['Matched Keywords']) {
                infoHTML += `<div class="modal-info-item"><span class="modal-info-label">Matched Keywords:</span> ${row['Matched Keywords']}</div>`;
            }
...
def load_data():
    ...
    try:
        # Look for the ASH Excel file in the same directory as this script
        script_dir = os.path.dirname(os.path.abspath(__file__))
        filename = '2025 ASH - Abstracts.xlsx'
        ...
        # Map ASH columns to standardized column names for display
        column_mapping = {
        ...
        # Combine abstract id and abstract order into a single Abstract # column
        # Format: "abs25-34 (5621)" or just "abs25-34" if order is missing
        df['Abstract #'] = df.apply(
            lambda row: f"{row['abstract id']} ({int(row['abstract order'])})"
            if pd.notna(row['abstract order']) and row['abstract order'] != 0
            else str(row['abstract id']),
            axis=1
        )

        # Keep Order as a separate column for sorting and display
        df['Order'] = df['abstract order'].fillna(0).astype(int)

        # Use Keywords as Track for compatibility
        df['Track'] = df['Keywords']
        ...
        # Sort by Order (abstract order) for proper sorting
...
def download_current_view():
    ...
    # Reorder columns to match display - ASH columns
    cols = ['Abstract #', 'Order', 'Abstract title', 'Category', 'Subcategory', 'Authors', 'First Author', 'Keywords', 'Abstract']
    ...
    filename = f"ash_abstracts_filtered_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
def download_results(task_id):
    ...
    # Reorder columns to match display - ASH columns
    base_cols = ['Abstract #', 'Order', 'Abstract title', 'Category', 'Subcategory', 'Authors', 'First Author', 'Keywords', 'Abstract']
...
if __name__ == '__main__':
    ...
    print("\n" + "="*50)
    print("ASH 2025 Abstract Annotator")
    print("="*50)
    print("\nStarting web server...")
